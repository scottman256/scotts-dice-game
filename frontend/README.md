...
The local `admin` / `admin` account has a separate Admin menu for enabling themes, resetting game data, and managing user accounts. Its global leaderboard view can add fictional system scores or remove individual entries. Classic cannot be disabled, and accounts authenticated through Google or Facebook do not expose password-change controls.
